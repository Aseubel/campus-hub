      ...
      <div class="lg:col-span-3 bg-white rounded-2xl shadow-soft p-6 animate-fade-in">
        ...
        <ul v-else class="space-y-3">
          <li 
            ...
            :key="category.id" 
            class="cursor-pointer p-3 rounded-xl transition-all duration-200 group"
            :class="{ 
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          >
            ...
            <div class="p-4">
              ...
              <div v-if="item.metadata" class="space-y-1 text-sm text-neutral-600">
                <div v-if="item.metadata.teacher" class="flex items-center">
                  <svg class="w-4 h-4 mr-1 text-primary-500" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd" />
                  </svg>
                  {{ item.metadata.teacher }}
                </div>
                <div v-if="item.metadata.credits" class="flex items-center">
                  <svg class="w-4 h-4 mr-1 text-primary-500" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M4 4a2 2 0 00-2 2v4a2 2 0 002 2V6h10a2 2 0 00-2-2H4zm2 6a2 2 0 012-2h8a2 2 0 012 2v4a2 2 0 01-2 2H8a2 2 0 01-2-2v-4zm6 4a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd" />
                  </svg>
                  {{ item.metadata.credits }} 学分
                </div>
                <div v-if="item.metadata.location" class="flex items-center">
                  <svg class="w-4 h-4 mr-1 text-primary-500" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd" />
                  </svg>
                  {{ item.metadata.location }}
                </div>
                <div v-if="item.metadata.openHours" class="flex items-center">
                  <svg class="w-4 h-4 mr-1 text-primary-500" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd" />
                  </svg>
                  {{ item.metadata.openHours }}
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-else-if="itemStore.currentItem" class="space-y-6">
            ...
            <div v-if="itemStore.currentItem.metadata" class="bg-primary-50 border border-primary-100 rounded-xl p-4">
              ...
              <div class="grid gap-3">
                <div v-for="(value, key) in itemStore.currentItem.metadata" :key="key" class="flex justify-between">
                  <span class="text-primary-700 font-medium">{{ formatMetadataKey(key) }}:</span>
              ...
              <div v-if="authStore.isLoggedIn" class="bg-white border border-neutral-200 rounded-xl p-4 mb-6">
                ...
                <div class="mb-4">
                  ...
                  <textarea 
                    ...
                    class="w-full px-3 py-2 border border-neutral-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-colors"
                  ></textarea>
                </div>
                ...
                >
                  <div class="flex items-start justify-between mb-3">
                    <div class="flex items-center">
                      ...
                      <div class="ml-3">
                        <p class="font-medium text-neutral-800">{{ review.userName }}</p>
